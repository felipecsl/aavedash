...
function weightedAverage(items: AssetPosition[], valueSelector: (item: AssetPosition) => number): number {
...
async function fetchFromAaveSubgraph(wallet: string): Promise<RawUserReserveWithMarket[]> {
  ...
  const failureText = marketResults
    ...
    .join(' | ');
  throw new Error(`Unable to fetch Aave user reserves from public endpoints. Tried: ${failureText}`);
}
...
function toAssetPosition(raw: RawUserReserve, amount: number, prices: Map<string, number>): AssetPosition {
...
function buildLoanPositions(reserves: RawUserReserveWithMarket[], prices: Map<string, number>): LoanPosition[] {
    ...
    <div className="app-shell">
      <main className="container">
        <header className="title-row">
          <div>
            <h1>Aave Loan Health Dashboard</h1>
            <p>Auto-fetched from wallet address using public blockchain data and price APIs.</p>
          </div>
        </header>

        <section className="card input-card">
          <form className="wallet-form" onSubmit={handleFetch}>
            <label className="field" htmlFor="wallet">
            ...
            <button className="button" type="submit" disabled={isLoading}>
              {isLoading ? <RefreshCw size={16} className="spin" /> : <Wallet size={16} />}
        ...
        {result ? (
          <>
            <article className="card summary-card">
              ...
              <p className="mono">{result.wallet}</p>
              <p className="muted small">Found {result.loans.length} active loan position(s)</p>
              <p className="muted small">Last updated: {new Date(result.lastUpdated).toLocaleString()}</p>
            </article>

            {result.loans.length > 0 ? (
              <>
                <nav className="tabs" aria-label="Loan positions">
                  {result.loans.map((loan, index) => (
                    <button
                      key={loan.id}
                      type="button"
                      className={`tab ${loan.id === selectedLoan?.id ? 'tab-active' : ''}`}
                ...
                <section className="dashboard-grid">
                  <Card>
                    ...
                    <CardBody>
                      ...
                      <div className="field">
                        <span>Supplied collateral assets</span>
                        <ul className="position-list">
                          {selectedLoan?.supplied.map((asset) => (
                            <li key={`${asset.address}-${asset.symbol}`}>
                  ...
                  <div className="right-column">
                    ...
                    <div className="split-grid">
                      ...
                      <Card>
                        ...
                        <CardBody>
                          ...
                          <Row label="Deploy earnings (annual)" value={fmtUSD(computed.deployEarnUSD, 0)} />
                          <Divider />
                    ...
                    <Card>
                      ...
                      <CardBody className="checklist-grid">
                        <ChecklistItem
                          title="Health Factor"
                          ok={!computed.alertHF}
        ...
        <footer className="muted small disclaimer">
          <p>
            Simplified monitor. Multi-collateral liquidation price is shown for the primary collateral
            asset only.
          </p>
        </footer>
      </main>
